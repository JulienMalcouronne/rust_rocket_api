...
#[get("/customer?<limit>&<page>", rank = 1)]
pub async fn get_customers(
    ...
    let my_page: i64 = limit.unwrap_or(12);

    match customer::find_customer(&db, my_limit, my_page).await {
        Ok(_doc) => Ok(Json(_doc)),
        Err(e) => {
            println!("Error = {:?}", e);
            Err("Error Occured")
        }
