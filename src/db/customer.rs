...
pub async fn insert_customer(db: &Database, input: Json<CustomerInput>) -> MongoResult<String> {
    ...
    let res = collection.insert_one(doc!("name": input.name.clone(), "city": input.city.clone(), "createdAt": created_at, "updatedAt": updated_at), None).await.unwrap();

    Ok(res.inserted_id.to_string())
}

pub async fn find_customer_by_id(db: &Database, _id: ObjectId) -> MongoResult<Option<Customer>> {
...
pub async fn find_customer(db: &Database, limit: i64, page: i64) -> MongoResult<Vec<Customer>> {
    let collection = db.collection::<CustomerDocument>("customer");
...
pub async fn update_customer_by_id(
    ...
    let find_res = find_customer_by_id(db, _id).await.unwrap();
    if find_res.is_none() {
        return Ok(None);
    }

    let doc = collection.find_one_and_update(doc!{"_id": _id}, doc!{"name": input.name.to_string(), "city": input.city.to_string(), "createdAt": find_res.unwrap().createdAt, "updatedAt": updated_at }, options).await.unwrap();
...
pub async fn delete_customer_by_id(db: &Database, _id: ObjectId) -> MongoResult<Option<Customer>> {
    let collection = db.collection::<CustomerDocument>("customer");
